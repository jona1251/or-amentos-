(function(){
  ...
  let permissions={...DEFAULT},primary=false,wrapped=false;
  ...
  function allowed(key){return primary||permissions[key]!==false}
  ...
  async function fetchAccess(){
    try{
      ...
      permissions={...DEFAULT,...(b.permissions||{})};primary=!!b.primaryAdmin;try{localStorage.setItem(cacheKey(),JSON.stringify({permissions,primary}))}catch(_){}
    }catch(e){try{const c=JSON.parse(localStorage.getItem(cacheKey())||'null');if(c){permissions={...DEFAULT,...(c.permissions||{})};primary=!!c.primary}}catch(_){} }
    window.orcaPermissions=permissions;window.orcaPrimaryAdmin=primary;apply();return {permissions,primary};
  }
  ...
  function canView(v){const p=viewPermission[v];if(p)return allowed(p);if(v==='suite')return allowed('operacoes')||allowed('crm')||allowed('relatorios')||allowed('financeiro')||allowed('seguranca');return true}
  ...
  function wrapSuiteTabs(){const old=window.suiteTab;if(typeof old!=='function'||old.__accessWrapped)return;const fn=async function(id){const map={ops:'operacoes',sales:'crm',manage:'relatorios',ai:'relatorios',security:'seguranca'};const p=map[id];if(p&&!allowed(p)){window.toast?.('Seu usuário não tem acesso a esta área Premium');return}return old(id)};fn.__accessWrapped=true;window.suiteTab=fn}
  ...
  function periodic(){wrapNavigation();wrapActions();wrapSuiteTabs();hookLogin();apply()}
