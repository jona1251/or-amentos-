  ...
  try {
    ...
    if (req.method === 'GET') {
      const data = await withRlsSql(company.id, user, async sql => {
        ...
        return rows.map(x => ({
          id:x.id,
          budgetId:x.budget_local_id,
          budgetNumber:x.budget_number,
          budgetStatus:x.budget_status,
    ...
    if (action === 'share_budget') {
      const budgetLocalId = String(req.body?.budgetId || '');
      ...
      let links = await adminSql`
        SELECT token FROM public_budget_links
      ...
      if (!token) {
        ...
        await adminSql`
          INSERT INTO public_budget_links(token, company_id, owner_user_id, budget_id)
          VALUES(${token}, ${company.id}, ${user.id}, ${budget.id})
          ON CONFLICT (company_id, owner_user_id, budget_id)
          DO UPDATE SET token=public_budget_links.token
        `;
    ...
    if (action === 'pix') {
      ...
      const data = await withRlsSql(company.id, user, async sql => {
        ...
        const s = await sql`
          SELECT pix_key, company_name, company_city
          FROM user_settings
          WHERE company_id=${company.id} AND user_id=${user.id}
          LIMIT 1
        `;
      ...
      const payload = buildPixPayload({
        key:data.settings.pix_key,
        merchantName:data.settings.company_name,
        merchantCity:data.settings.company_city,
        amount:data.budget.total,
        description:`Orcamento ${data.budget.number}`,
    ...
    if (action === 'create_receivable') {
      ...
      const result = await withRlsSql(company.id, user, async sql => {
        ...
        const rows = await sql`
          ...
          VALUES(${company.id}, ${user.id}, ${budget.id}, ${Number(budget.total || 0)}, ${dueDate}, 'Pendente', now())
          ON CONFLICT (company_id, owner_user_id, budget_id)
    ...
    if (action === 'update_receivable') {
      ...
      const rows = await withRlsSql(company.id, user, async sql => {
        const current = await sql`
          SELECT id, amount FROM receivables
          WHERE id=${id} AND company_id=${company.id} AND owner_user_id=${user.id}
          LIMIT 1
        `;
    ...
    return send(res, 500, { ok:false, error:'SERVER_ERROR', detail:String(err?.message || '').slice(0,160) });
  }
};
