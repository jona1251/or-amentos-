...
module.exports = async function handler(req, res) {
  try {
    ...
    if (req.method === 'PATCH') {
      ...
      const pinHash = String(req.body?.pinHash || '');
      if (!id || !validPinHash(pinHash)) return send(res, 400, { ok:false, error:'INVALID_REQUEST' });
      const rows = await sql`
        UPDATE users SET pin_hash=${pinHash}
        WHERE company_id=${company.id} AND id::text=${id}
        RETURNING id
      `;
      if (!rows.length) return send(res, 404, { ok:false, error:'USER_NOT_FOUND' });
