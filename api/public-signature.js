...
module.exports=async function handler(req,res){
  try{
    ...
    if(req.method==='POST'){
      ...
      await withRlsSql(link.company_id,{id:link.owner_user_id,role:'operador'},async rls=>{
      ...
      await sql`UPDATE suite_public_links SET used_at=now(),payload=payload||${JSON.stringify({signed:true})}::jsonb WHERE token=${token}`;
      return send(res,200,{ok:true});
    }

    if(req.method!=='GET')return send(res,405,{ok:false,error:'METHOD_NOT_ALLOWED'});
    let title=String(link.payload?.title||'Assinatura digital'),subtitle=String(link.payload?.subtitle||'Revise e assine abaixo.');
...
function clearSig(){ctx.clearRect(0,0,c.width,c.height);moved=false}async function submitSig(){const name=document.getElementById('name').value.trim();if(!name)return alert('Informe o nome completo.');if(!moved)return alert('Faça sua assinatura no quadro.');if(!confirm('Confirmar esta assinatura?'))return;const signature=c.toDataURL('image/png');try{const r=await fetch(location.href,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({signerName:name,signature})});const d=await r.json();if(!r.ok)throw new Error(d.error||'Falha');document.querySelector('.card').innerHTML='<h1>Assinatura registrada</h1><div class="ok">✓ Documento assinado com sucesso.</div><p class="muted">A empresa recebeu sua assinatura, data e registro técnico de confirmação.</p>'}catch(e){alert('Não foi possível registrar a assinatura. Tente novamente.') }}
</script>`}</body></html>`;
    res.status(200).setHeader('Content-Type','text/html; charset=utf-8');res.setHeader('Cache-Control','no-store');return res.send(html);
