...
module.exports=async function handler(req,res){
  try{
    ...
    const data=await withRlsSql(link.company_id,{id:link.owner_user_id,role:'operador'},async rls=>{
      ...
      let amount=Number(link.payload?.amount||0),title=String(link.payload?.title||'Pagamento'),clientName=String(link.payload?.clientName||'');
      ...
      return {settings:s[0]||{},amount,title,clientName};
    });
    let payload='',qr='';
