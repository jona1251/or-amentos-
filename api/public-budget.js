...
function expired(link) { return !!link?.expires_at && new Date(link.expires_at).getTime() <= Date.now(); }
...
function securityHeaders(res) {
  ...
  res.setHeader('Content-Security-Policy',"default-src 'self' data: https:; style-src 'unsafe-inline'; script-src 'unsafe-inline'; img-src 'self' data: https:; connect-src 'self'; frame-ancestors 'none'; base-uri 'none'; form-action 'self'");
}
function resultPage(title,message,icon='✓') {
  return `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${e(title)}</title><style>*{box-sizing:border-box}body{margin:0;background:#f3f5f8;font-family:Inter,Arial,sans-serif;color:#172033;display:grid;min-height:100vh;place-items:center;padding:18px}.box{width:min(92vw,560px);background:white;border:1px solid #e5e7eb;border-radius:22px;padding:30px;box-shadow:0 18px 50px rgba(15,23,42,.08);text-align:center}h1{font-size:24px;margin:0 0 10px}p{color:#64748b;line-height:1.6}.ok{font-size:46px}</style></head><body><div class="box"><div class="ok">${e(icon)}</div><h1>${e(title)}</h1><p>${e(message)}</p></div></body></html>`;
}
...
async function loadPublicData(link) {
  return withRlsSql(link.company_id,{id:link.owner_user_id,role:'operador'},async sql=>{
    ...
    const items = await sql`SELECT description,quantity,unit_price,total FROM budget_items WHERE budget_id=${link.budget_id} ORDER BY id`;
    return {...rows[0],items};
  });
}
...
module.exports = async function handler(req,res) {
  try {
    ...
    if (!validToken(token)) {
      if (req.method === 'GET') return sendHtml(res,404,resultPage('Link não encontrado','Confira se o endereço recebido está completo.','!'));
      return send(res,404,{ok:false,error:'LINK_NOT_FOUND'});
    }
    const adminSql = getSql();
    await ensurePremiumSchema(adminSql);
    const links = await adminSql`
      SELECT token,company_id,owner_user_id,budget_id,response_status,responded_at,expires_at
      FROM public_budget_links WHERE token=${token} LIMIT 1
    `;
    const link = links[0];
    if (!link) {
      if (req.method === 'GET') return sendHtml(res,404,resultPage('Link não encontrado','Este orçamento não está mais disponível.','!'));
      return send(res,404,{ok:false,error:'LINK_NOT_FOUND'});
    }
    if (expired(link)) {
      if (req.method === 'GET') return sendHtml(res,410,resultPage('Link expirado','Solicite à empresa um novo link para consultar este orçamento.','⌛'));
      return send(res,410,{ok:false,error:'LINK_EXPIRED'});
    }
    ...
    if (req.method === 'POST') {
      ...
      if (link.response_status) return send(res,409,{ok:false,error:'ALREADY_RESPONDED',status:link.response_status});

      try {
        const outcome = await withRlsSql(link.company_id,{id:link.owner_user_id,role:'operador'},async sql=>{
          // Faz a "reserva" da resposta dentro da mesma transação. Em cliques repetidos ou
          // concorrentes apenas uma requisição consegue responder.
          const claimed = await sql`
            UPDATE public_budget_links
            SET responded_at=now(),response_status=${status}
            WHERE token=${token} AND response_status IS NULL AND expires_at>now()
            RETURNING id
          `;
          ...
          `;
          if (!budgets.length) {
            const err = new Error('BUDGET_NOT_FOUND');err.code='BUDGET_NOT_FOUND';throw err;
          }
          if (action === 'approve') {
            await sql`
              INSERT INTO receivables(company_id,owner_user_id,budget_id,amount,status,updated_at)
              VALUES(${link.company_id},${link.owner_user_id},${link.budget_id},${Math.max(0,Number(budgets[0].total||0))},'Pendente',now())
        ...
        if (!outcome.claimed) return send(res,409,{ok:false,error:'ALREADY_RESPONDED'});
      } catch (err) {
        if (err?.code === 'BUDGET_NOT_FOUND' || err?.message === 'BUDGET_NOT_FOUND') return send(res,404,{ok:false,error:'BUDGET_NOT_FOUND'});
        throw err;
      }
    ...
    const data = await loadPublicData(link);
    if (!data) return sendHtml(res,404,resultPage('Orçamento não encontrado','O documento vinculado a este link não está mais disponível.','!'));
    ...
    if (data.pix_key) {
      pixPayload = buildPixPayload({
        key:data.pix_key,merchantName:data.company_name,merchantCity:data.company_city,amount:data.total,
        description:`Orcamento ${data.number}`,txid:String(data.number||'ORC').replace(/[^A-Za-z0-9]/g,'').slice(0,25)||'***'
      });
